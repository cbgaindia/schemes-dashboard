...
export async function fetchNews(query) {
  ...
  const currentScheme = Object.keys(SchemesData).find(
    (eachScheme) => SchemesData[eachScheme].dataId == query
  );

  if (result[currentScheme]) {
    while (result[currentScheme].length) {
      recentDevelopmentsArray.push(result[currentScheme].splice(0, 2));
    }
  }
